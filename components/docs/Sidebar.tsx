...
function renderBlock(block: SidebarBlock, pathStr: string) {
  return (
    <div key={block.titleKey} className="mb-8">
      <p className="mb-3 text-[11px] font-medium uppercase tracking-wider text-notifyy-muted dark:text-notifyy-mutedDark">
...
export function Sidebar({ className }: { className?: string }) {
  ...
  return (
    <aside
      className={cn(
        "w-64 shrink-0 border-r border-gray-200/90 bg-notifyy-card p-5 dark:border-notifyy-borderDark dark:bg-notifyy-cardDark lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto",
        className,
      )}
    >
